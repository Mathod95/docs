---
title: "Helm Charts, ArgoCD, and ECR"
date: 2026-01-18
categories:
  - "uncategorized"
tags:
  - "untagged"
source: "https://medium.com/@hans.knechtions/helm-charts-argocd-and-ecr-beceec0accc1"
author:
  - "[[Sven Hans Knecht]]"
---
<!-- more -->
...
# Authenticate to Amazon ECR
      - name: Login to Amazon ECR
        uses: aws-actions/amazon-ecr-login@v1
```
- Install helm and [helm-docs](https://github.com/norwoodj/helm-docs) (Helm docs is a very useful helm tool that qill auto generate the readme for your chart).
```c
- name: Install Helm
        uses: azure/setup-helm@v3.5
        with:
...
[Install documentation](https://external-secrets.io/latest/introduction/getting-started/) is reasonably complete for external-secrets. There’s only a few things to add.

First, create an IAM Policy and grant it access to ECR. I didn’t make this policy particularly locked down, but you can limit it to only read actions if you’d like. You may also want to add SecretsManager permissions to the policy because External Secrets is a very useful tool in general.
...
Second, create an IAM role that trusts the external-secrets Kubernetes service account and attach the policy you just created. Here is an example of the trust relationship.json. [AWS](https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html) has more documentation if needed.
...
This yaml uses the [ECR Authorization Token Generator](https://external-secrets.io/latest/api/generator/ecr/) to rotate and create tokens that ArgoCD can use and then puts it in a [format](https://argo-cd.readthedocs.io/en/stable/operator-manual/argocd-repo-creds-yaml/) that ArgoCD can understand to use for the helm login command.
...
](https://medium.com/?source=post_page---read_next_recirc--beceec0accc1---------------------------------------)