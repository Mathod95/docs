...
tags:
  - Grafana Tempo
source: "https://medium.com/@nsalexamy/deploying-grafana-tempo-on-kubernetes-a-practical-guide-20708ed61c0b"
author:
  - "[[Young Gyu Kim]]"
---
<!-- more -->
...
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*B9BK9idFRyPhzbAgOZfgSA.png)
...
1. OTLP Receiver in Otel Collector handles Traces from applications
2. The OTLP/Jaeger Exporter sends the traces to Jaeger V2 Collector
3. Jaeger V2 Collector stores traces into Cassandra Cluster
...
The `backend` is set to `s3`, and the S3 bucket, endpoint, region, and prefix are specified for trace storage.The `extraEnvFrom` section is used to reference the Kubernetes secret containing AWS credentials, allowing Tempo to access the S3 bucket.
...
The `otlp/tempo` exporter is configured to send traces to Tempo using the OTLP protocol. The endpoint is set to the Tempo service URL.The 'otlp/tempo' exporter is added to the traces pipeline, allowing the OpenTelemetry Collector to send traces to Tempo.
...
- **Name**: tempo
- **URL**: [http://tempo.o11y.svc:3100](http://tempo.o11y.svc:3100/)

## Exploring Traces in Grafana

![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*QtKIMAyl2dmt9C4YbSWtgg.png)

Figure 1. Grafana UI — Data Sources

## Exploring Traces in Grafana

Go to Explore, select the Tempo data source, and run trace queries.

![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*jLjXZwzOCQfT53_Sp8T5rQ.png)

Figure 2. Grafana UI — Tempo Traces

Click on any trace to inspect span details, attributes.

![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*TJyqaT4lFRexLTY6-BT7QA.png)

Figure 3. Grafana UI — Tempo Trace Details
...
- [https://nsalexamy.github.io/service-foundry/pages/documents/o11y-foundry/install-tempo-on-k8s/](https://nsalexamy.github.io/service-foundry/pages/documents/o11y-foundry/install-tempo-on-k8s/)
...
](https://medium.com/?source=post_page---read_next_recirc--20708ed61c0b---------------------------------------)