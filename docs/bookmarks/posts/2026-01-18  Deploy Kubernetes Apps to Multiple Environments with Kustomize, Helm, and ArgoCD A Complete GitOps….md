---
title: "Deploy Kubernetes Apps to Multiple Environments with Kustomize, Helm, and ArgoCD: A Complete GitOps…"
date: 2026-01-18
categories:
  - "uncategorized"
tags:
  - untagged
source: https://medium.com/@nsalexamy/deploy-kubernetes-apps-to-multiple-environments-with-kustomize-helm-and-argocd-a-complete-gitops-be760b222dcc
author:
  - "[[Young Gyu Kim]]"
---
<!-- more -->
...
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*BBH9eb4r1z3XI_6qC5-sbw.png)

## Introduction

YouTube Tutorial: [Deploy Multi-Environment Apps with Kustomize, Helm & ArgoCD | GitOps Tutorial](https://youtu.be/o7s0VGVSeuE)
...
We’ll deploy the `service-foundry-community` Helm chart (which we created in the [previous article](https://nsalexamy.github.io/service-foundry/pages/documents/blog/helm-chart-with-multiple-apps/)) to three different environments using Kustomize overlays and ArgoCD. This chart contains:
...
- **Development**: [https://community-dev.servicefoundry.org](https://community-dev.servicefoundry.org/)
- **Staging**: [https://community-staging.servicefoundry.org](https://community-staging.servicefoundry.org/)
- **Production**: [https://community.servicefoundry.org](https://community.servicefoundry.org/)
...
```
```c
routes:    - match: Host(\`{{ .Values.host }}\`) && PathPrefix(\`/api\`)
      kind: Rule
      services:
        - name: {{ include "service-foundry-community.backendFullname" . }}
          port: http
      middlewares:
        - name: api-stripprefix    - match: Host(\`{{ .Values.host }}\`) && PathPrefix(\`/\`)
      kind: Rule
      services:
...
For more details on how this chart was created, check out the [video tutorial](https://youtu.be/mGtIVDMaUkg) or the [written guide](https://nsalexamy.github.io/service-foundry/pages/documents/blog/helm-chart-with-multiple-apps/).
...
$ rm -rf chart-home/service-foundry-community# Pull and extract the chart from ECR
$ helm pull \
...
```
```c
namespace: dev  # 1resources:
  - service-foundry-license-keys-dev-sealed.yaml  # 2helmGlobals:  # 3
  chartHome: ../chart-homehelmCharts:  # 4
...
```
```c
host: community-dev.servicefoundry.org  # 2frontend:
  config:
    enabled: true
    content: |  # 3
      {
...
source:
    ...
    targetRevision: main
    path: sf-apps/service-foundry-community/dev  # 3  destination:
...
template:  # 2
    ...
    spec:
      project: default      source:
        ...
        targetRevision: main
        path: 'sf-apps/service-foundry-community/{{env}}'  # 4      destination:
        server: https://kubernetes.default.svc
        namespace: '{{namespace}}'  # 5      syncPolicy:
        automated:
          prune: true
          selfHeal: true
          allowEmpty: false        syncOptions:
...
3. **{browser}**: Template variable that gets replaced with `dev`, `staging`, or `prod`
...
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*E9YwHt9ccD-GnjWMgdAdpw.png)

Figure 1. ArgoCD UI: service-foundry-community-dev
...
[https://community-dev.servicefoundry.org](https://community-dev.servicefoundry.org/)

![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*t4UO2iU-Qyuc2OVozQK3Yg.png)

Figure 2. Service Foundry Community — Development Environment
...
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*xJZ3De_erTZi2_PtBGLzRg.png)

Figure 3. ArgoCD UI: All Service Foundry Community Environments

ArgoCD manages three separate applications:

- **service-foundry-community-dev** → [https://community-dev.servicefoundry.org](https://community-dev.servicefoundry.org/)
- **service-foundry-community-staging** → [https://community-staging.servicefoundry.org](https://community-staging.servicefoundry.org/)
- **service-foundry-community-prod** → [https://community.servicefoundry.org](https://community.servicefoundry.org/)

## Production is Live!

Here’s the production environment running:

![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*YjJkyiZL5cyi-k5w62QxUA.png)

Figure 4. Service Foundry Community — Production Environment
...
1. **Kustomize cannot access private OCI registries**: Pull charts locally and commit them to Git for Kustomize to use
2. **SealedSecrets are namespace-scoped**: Create separate sealed secrets for each namespace — they cannot be shared
3. **ApplicationSets reduce duplication**: Use them to manage similar applications across multiple environments
4. **GitOps in action**: All configuration is versioned in Git, and ArgoCD ensures the cluster matches the desired state
...
- [Creating the Service Foundry Helm Chart](https://nsalexamy.github.io/service-foundry/pages/documents/blog/helm-chart-with-multiple-apps/)
- [Video Tutorial: Multi-App Helm Charts](https://youtu.be/mGtIVDMaUkg)
...
](https://medium.com/?source=post_page---read_next_recirc--be760b222dcc---------------------------------------)