---
title: "New Dockerfile capabilities in v1.7.0"
date: 2026-01-19
categories:
  - "uncategorized"
tags:
...
We recently released the new versions of the , , and [Dockerfile frontend for BuildKit (v1.7.0)](https://hub.docker.com/r/docker/dockerfile). In this blog post, I’ll go over some of the new Dockerfile capabilities and explain how you can start using them in your projects.

![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*A4D403GqBkvpxqxR8JZI8g.png)
...
When you use such variables, you most likely use `${NAME}` or, more simplified, `$NAME` in `COPY`, `RUN`, and other commands.
...
One of the techniques described in [“Advanced multi-stage build patterns”](https://medium.com/@tonistiigi/advanced-multi-stage-build-patterns-6f741b852fae) shows how build arguments can be used so that different Dockerfile commands run depending on the build-arg value. For example, you can use that pattern if you build a multi-platform image and wish to run additional `COPY` or `RUN` commands only for specific platforms.
...
](https://medium.com/?source=post_page---read_next_recirc--be6873650741---------------------------------------)