---
title: "Stressing a Kubernetes Pod to Induce an OOMKilled Error"
date: 2026-01-19
categories:
  - "uncategorized"
tags:
...
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*x1kvMGh4HnKmBJZCGmmf9Q.png)
...
When the OOMKilled error occurs, Kubernetes captures this event and reports it in the container’s status. You can inspect the container’s logs or query Kubernetes using commands like `kubectl describe pod` or `kubectl logs` to gather more information about the OOMKilled event. The logs will typically include details about the container, the reason for termination (out of memory), and the exit status.
...
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*OMDe-n4cX3Ndw8v63ynVaQ.png)
...
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*gpOYLf2RvajlVWY-F5mk6Q.png)
...
This manifest file will create a Pod resource in the `mem-example` namespace that we just created. The **spec** section defines a single container to be created from the image `polinux/stress`, which is an image commonly used for generating workload and stress testing. Under the `resources` section, we define the resource requirements and limits for the container. We are requesting `100Mi` of memory (100 MB) and setting the maximum amount of resource memory as `200Mi` (200 MB). The `command` specifies the command to be executed within the container, which will be a stress command. The `args` provide additional arguments to the command, telling Kubernetes to create a single virtual memory workload of 150 MB and hang for 1 second.

## Apply pod.yaml to the Kubernetes Cluster

Use the `kuebctl apply` command to apply the pod.yaml manifest to the cluster. The `-f` flag indicates which file to apply and `--namespace=` tells Kubernetes which namespace to place the pod in.

```c
kubectl apply -f pod.yaml --namespace=mem-example
```
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*CrBwDep-WC5br9SgufEAdw.png)
...
![](https://miro.medium.com/v2/resize:fit:640/format:webp/0*Ws8AOA0rM1D-WVvB.gif)
...
kubectl get pods
```
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*wNKyH3QxQyhUcVibtSFtRQ.png)
...
```
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*tjuaVCQrX-OQ-smJRIRRug.png)
...
```
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*svIYDNS5GQf6vEaEU88wBg.png)

If you get an error, try using `sudo` before the command:

```c
sudo kubectl top pod memory-demo
```
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*dcygPeeJnTL-UVzTDuj6VQ.png)
...
```
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*W-8w8t931lfR_CzCtMStzA.png)

## Create a Pod that Exceeds the Set Memory Limits

Using the same pod.yaml manifest as before, let’s edit the `args` section and change the `"150M"` to `"250M"`. By changing this argument to 250 MB, we are telling Kubernetes to stress the container and attempt to allocate 250 MB of memory, which is more than the set limit of 200 MB. This should induce an OOMkilled error.
...
kubectl get pod
```
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*WNaKcSQf71KoZ54sND9EoA.png)
...
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*D75JEtbbqL6SprHk-HM53A.png)
...
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*FMjuIam3R7e_L3mLd0Nn_g.png)
...
```
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*osIjWhhdrZlFHRQitlyrjg.png)
...
```
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*xywlJ48dLM4eR9ZDzTlrvQ.png)

Thank you so much for following along with this tutorial and continue watching for more DevOps how-tos!

![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*CKS7crh9SK4db60TlMcUSw.png)

Nurse Practitioner turned DevOps Engineer on a journey of seeking and sharing knowledge

## More from Brandi McCall
...
](https://medium.com/?source=post_page---read_next_recirc--96f3be9c931d---------------------------------------)