...
## Labels
https://kubernetes.io/docs/concepts/overview/working-with-objects/common-labels/?utm_source=chatgpt.com
...
🔹 Outils de déploiement / gestion
kubectl → ressource appliquée manuellement avec kubectl apply
Helm → ressource déployée via un chart Helm
Kustomize → ressource générée via Kustomize
ArgoCD → ressource gérée par ArgoCD GitOps
Flux → ressource gérée par Flux GitOps
Terraform → ressource provisionnée via Terraform Kubernetes provider
Pulumi → ressource provisionnée via Pulumi

🔹 Outils de CI/CD
Jenkins → appliqué via pipeline Jenkins
GitHub Actions → appliqué via workflow GitHub Actions
GitLab CI → appliqué via pipeline GitLab

🔹 Outils de monitoring ou de gestion d’infrastructure
Rancher → ressource gérée via Rancher
OpenShift → ressource gérée par la plateforme OpenShift
Kapp → outil de déploiement Carvel
Helmfile → orchestrateur de charts Helm
...
🔹 Composants front-end / interface utilisateur

frontend – interface utilisateur principale

web – site web ou portail web

ui – interface graphique

🔹 Composants back-end / application

backend – logique métier

api – serveur API

worker – traitement en arrière-plan

service – microservice générique

🔹 Base de données et stockage

database – base de données (ex. MySQL, PostgreSQL)

cache – cache (ex. Redis, Memcached)

queue – gestionnaire de messages (ex. RabbitMQ, Kafka)

storage – stockage de fichiers (ex. MinIO)

🔹 Infrastructures et utilitaires

proxy – reverse proxy (ex. Nginx, Traefik)
...
scheduler – orchestrateur interne

job – tâches batch ou planifiées

🔹 Autres exemples courants

auth – gestion des authentifications

messaging – gestion de messages

notification – service de notifications

analytics – collecte et traitement de données